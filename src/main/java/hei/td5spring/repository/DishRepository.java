...
public class DishRepository {
    ...
    public List<Dish> findAll() {
        List<Dish> dishes = jdbc.query("SELECT * FROM dish",
                (rs, rowNum) -> new Dish(
                        rs.getLong("id"),
                        rs.getString("name"),
        ...
        return dishes.stream().map(d -> {
            List<Ingredient> ingredients = jdbc.query(
                    "SELECT i.* FROM ingredient i " +
                            ...
                            "WHERE di.dish_id = ?",
                    ingredientRepository.ingredientRowMapper,   // maintenant accessible
                    d.id()
            );
            return new Dish(d.id(), d.name(), d.sellingPrice(), ingredients);
        }).toList();
    }
    ...
    public void updateIngredients(Long dishId, List<Ingredient> requestedIngredients) {
        ...
        List<Long> existingIds = requestedIngredients.stream()
                .map(Ingredient::id)
                .distinct()
                .toList();

        String inClause = existingIds.isEmpty() ? "0" :
                existingIds.stream().map(String::valueOf).reduce((a, b) -> a + "," + b).get();

        List<Long> realIds = jdbc.query(
                "SELECT id FROM ingredient WHERE id IN (" + inClause + ")",
                (rs, rowNum) -> rs.getLong("id")
        );

        for (Long ingId : realIds) {
